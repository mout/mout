...
describe('queryString/decode()', function() {
    it('should decode query string and typecast values', function() {
        const q = decode('?a=123&b=false&c=null&d=bar');
        expect(q.a).toBe(123);
    ...
    it('should decode special chars', function() {
        const q = decode('?a=bar&b=lorem%20ipsum&c=sp%C3%A9%C3%A7%C3%AE%C3%A3l%20%C3%A7h%C3%A2rs');
        expect(q.a).toBe('bar');
    ...
    it('should allow bypassing the typecast', function() {
        const q = decode('?a=123&b=false&c=null&d=bar', false);
        expect(q.a).toBe('123');
        expect(q.b).toBe('false');
        expect(q.c).toBe('null');
        expect(q.d).toBe('bar');
    ...
    it('should allow properties with same name and create an array', function() {
        const q = decode(
            '?a=undefined&a=false&a=0&a=null&a=bar&b=123&b=foo&c=loren&d=&d=&d=sp%C3%A9%C3%A7%C3%AE%C3%A3l%20%C3%A7h%C3%A2rs'
        );
    ...
    it('should allow equal signs inside property values', function() {
        const q = decode('?lorem=ipsum=dolor==Xds&sit=amet');
